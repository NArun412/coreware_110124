...
if (file_exists("apc.conf.php")) /** @noinspection PhpIncludeInspection */ {
...
echo <<<EOB
		...
		<div class="info div2"><h2>Runtime Settings</h2><table><tbody>
EOB;
...
if ($mem['num_seg'] > 1 || $mem['num_seg'] == 1 && count($mem['block_lists'][0]) > 1) {
	$mem_note = "Memory Usage<br /><p>(multiple slices indicate fragments)</p>";
} else {
...
if (isset($mem['adist'])) {
	foreach ($mem['adist'] as $i => $v) {
		...
		echo "<tr><th style='text-align: right'>$range</th><td style='text-align: right'>$v</td></tr>\n";
	}
}
?>
</tbody></table>
...
<br/>
<?php
if (empty($_GET['show_list'])) {
	break;
}
?>
<div class="info"><h2>Listing of cache keys</h2>
    <table>
        <tbody>
		<?php
		if (class_exists("APCUIterator", false)) {
			$oddRow = false;
			$cacheValues = array();
			foreach (new APCUIterator("/.*/") as $apcuCache) {
				$cacheValue = apcu_fetch($apcuCache['key']);
				$cacheSize = strlen(is_object($cacheValue) ? serialize($cacheValue) : (is_array($cacheValue) ? json_encode($cacheValue) : $cacheValue));
				if ($cacheSize < 100000) {
					continue;
				}
				$cacheValues[] = array("key" => $apcuCache['key'], "size" => $cacheSize);
			}
			usort($cacheValues, "sortCache");
			foreach ($cacheValues as $thisValue) {
				$oddRow = !$oddRow;
				echo "<tr class='tr-" . ($oddRow ? "1" : "0") . "'><td>" . $thisValue['key'] . "</td><td>" . $thisValue['size'] . "</td></tr>";
			}
		}
		echo <<<EOB
		</tbody></table>
