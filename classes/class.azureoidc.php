...
class AzureOidc extends OidcProvider {

    private $iAccessToken;

    public function __construct($loginProviderCredentialId) {
        parent::__construct($loginProviderCredentialId);
        $this->iCallback = ltrim(getPreference("MICROSOFT_LOGIN_CALLBACK"),"/ ") ?: "callback";
    }

    function getLoginText() {
        return getPreference("MICROSOFT_LOGIN_LOGIN_TEXT") ?: "Login with Microsoft";
    }

    function getLoginFragment() {
        return getFragment("MICROSOFT_LOGIN_FRAGMENT") ?: parent::getLoginFragment();
    }
    ...
    function getLoginUrl() {
        ...
        $stateKey = "azure_oidc_state_" . $_SERVER['REMOTE_ADDR'];
        ...
        $loginUrl = sprintf("%s?client_id=%s&response_type=code+id_token&response_mode=form_post&scope=%s&redirect_uri=%s&nonce=%s&state=%s",
            $this->iLoginUrl,
            $this->iClientId,
    ...
    public function processUserInfo($parameters) {
        ...
        $result['user_name'] = $idToken['preferred_username'];
        return $result;
    }
    ...
    function getIdToken($authorizationCode) {
        ...
        $err = curl_error($curl);
        if ($this->iLogging) {
            $logEntry = "Azure OIDC Token request: " . http_build_query($data) .
                "\nAzure OIDC Token response: " . $result;
            $logEntry .= (empty($err) ? "" : "\nAzure OIDC Token Error: " . $err);
        ...
        if($this->iLogging) {
            addDebugLog("Azure OIDC ID Token: " . jsonEncode($idToken));
        }
        return $idToken;
    }
    ...
    public function getUserInfo() {
        ...
        $err = curl_error($curl);
        if ($this->iLogging) {
            $logEntry = "Azure OIDC request: " . $this->iUserInfoUrl .
                "\nAzure OIDC response: " . $result;
            $logEntry .= (empty($err) ? "" : "\nAzure OIDC Token Error: " . $err);
            addDebugLog($logEntry);
        }
        if ($result === false || !in_array($info['http_code'], [200,201,202])) {
