...
function customActions(actionName) {
	if (actionName == "clearlog") {
		document.location = "<?= $GLOBALS['gLinkUrl'] ?>?url_action=clearlog";
		return true;
	}
	return false;
}
<?php
	...
	function executePageUrlActions() {
		if ($_GET['url_action'] == "clearlog" && $GLOBALS['gPermissionLevel'] > 1) {
			executeQuery("delete from security_log");
		}
	}
}

$pageObject = new ThisPage("security_log");
