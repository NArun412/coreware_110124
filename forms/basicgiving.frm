...
<?php
	...
	if (!empty($customPresetAmount) && is_numeric($customPresetAmount)) {
		...
		while (true) {
			...
			$presetAmounts[] = array("amount"=>$presetAmount,"text"=>$presetText);
		}
	}
...
?>
	<button tabindex="10" class="preset-amount" data-amount="<?= $thisAmount ?>"><?php if (!empty($thisAmount) && $thisAmount > 0) { ?>$<?= $thisAmount ?><?php } ?><?= (empty($thisDescription) ? "" : ((!empty($thisAmount) && $thisAmount > 0) ? "<br>" : "") . $thisDescription) ?></button>
<?php
			}
?>
<?php if (empty($customPresetAmount)) { ?>
	<button tabindex="10" class="preset-amount" data-amount="">Other</button>
<?php } ?>
...
<?php
		...
		$resultSet = executeQuery("select * from designations where " . $query . " order by sort_order,description",$parameters);
		while ($row = getNextRow($resultSet)) {
			$designationArray[$row['designation_id']] = array("designation_code"=>$row['designation_code'],"alias"=>$row['alias'],"description"=>$row['description']);
		}
...
<div class="form-line" id="_last_name_row">
	...
	<input tabindex="10" type="text" <?= ($GLOBALS['gUserRow']['administrator_flag'] ? "readonly='readonly'" : "class='validate[required]" . (in_array("first_name",$capitalizedFields) ? " capitalize" : "") . "'") ?> size="30" maxlength="35" id="last_name" name="last_name" placeholder="Last Name" value="<?= htmlText($userRow['last_name']) ?>">
	<div class='clear-div'></div>
</div>
...
<div class="form-line" id="_billing_postal_code_row">
	...
	<span class="help-label">for billing</span>
