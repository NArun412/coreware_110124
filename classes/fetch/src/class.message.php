...
class Message
{
	...
	public function getMessageBody($html = false) {
		$ignoreHeaders = array("From","Reply-To","Date","To","Subject","X-Microsoft-Exchange-Diagnostics","X-Microsoft-Antispam-Message-Info");
		if ($html) {
			...
			if (!isset($this->plaintextMessage) && isset($this->htmlMessage)) {
				...
				$output = $this->plaintextMessage;
			}
			$messageLines = explode("\n",$output);
			$editedMessageBody = "";
			$useLine = true;
			foreach ($messageLines as $thisLine) {
				$thisLine = trim($thisLine);
				if (substr($thisLine,0,2) == "> " || $thisLine == ">") {
					continue;
				}
				foreach ($ignoreHeaders as $thisHeader) {
					if (substr($thisLine,0,strlen($thisHeader . ": ")) == $thisHeader . ": ") {
						continue;
					}
				}
				if (!stristr($thisLine,"forwarded") && substr($thisLine,0,10) == "----------") {
					$useLine = !$useLine;
				}
				if (!$useLine) {
					continue;
				}
				$editedMessageBody .= $thisLine . "\n";
			}
			$editedMessageBody = preg_replace("/(\r?\n){2,}/", "\n\n", $editedMessageBody);
			$editedMessageBody = str_replace("\n\n","<doublenl>",$editedMessageBody);
			$editedMessageBody = str_replace("\n","<nl>",$editedMessageBody);
			$editedMessageBody = str_replace("<doublenl>","\n\n",$editedMessageBody);
			$messageLines = explode("\n",$editedMessageBody);
			$editedMessageBody = "";
			$useLine = true;
			foreach ($messageLines as $thisLine) {
				...
				if (substr($thisLine,0,strlen("From: ")) == "From: ") {
					continue;
				}
				if (substr($thisLine,0,strlen("Reply-To: ")) == "Reply-To: ") {
					continue;
				}
				if (substr($thisLine,0,strlen("Date: ")) == "Date: ") {
					continue;
				}
				if (substr($thisLine,0,strlen("To: ")) == "To: ") {
					continue;
				}
				if (substr($thisLine,0,strlen("Subject: ")) == "Subject: ") {
					continue;
				}
				if (substr($thisLine,0,strlen("On ")) == "On " && substr($thisLine,-6) == "wrote:") {
					continue;
				}
                if (!stristr($thisLine,"forwarded") && substr($thisLine,0,10) == "----------") {
					$useLine = !$useLine;
			...
			$messageBody = str_replace("<nl>","\n",$editedMessageBody);
			$messageBody = trim(html_entity_decode($messageBody));
			do {
				$length = strlen($messageBody);
				$messageBody = str_replace("\r\r\r","\r\r",$messageBody);
				$newLength = strlen($messageBody);
	...
	public function getAddresses($type="all", $asString = false)
	{
		...
		foreach ($useTypes as $type) {
			...
			if (!$asString) {
				foreach ($this->$type as $emailInfo) {
					if (!in_array($emailInfo['address'],$addresses)) {
						$addresses[] = $emailInfo['address'];
					}
				}
			} else {
				foreach ($this->$type as $address) {
					$outputString .= (empty($outputString) ? "" : ", ") .isset($address['name']) ? $address['name'] . ' <' . $address['address'] . '>' : $address['address'];
				}
			}
		}
		return ($asString ? $outputString : $addresses);
	}
	 ...
	 * mailbox until the Imap->expunge is run.
	 *
	 * @return bool
	 */
